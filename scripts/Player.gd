...
func _process(delta: float) -> void:
	...
	if hooked:
		if Input.is_action_pressed("accel ccw") and hooked:
			...
			Power -= delta * 2.
		elif Input.is_action_pressed("accel cw") and hooked:
			var accel_vector = (position - HookPoint.position).normalized()
			accel_vector = Vector2(-accel_vector.y, accel_vector.x)
			apply_force(accel_vector * 10000.)
			Power -= delta * 2.
		
		#var connection = HookPoint.position - position
		#if abs(connection.length() - PullRange) > 5:
		#	position += connection.normalized() * 1000. * delta
...
func _draw() -> void:
	...
	var fac = linear_velocity.length() / 4000.
