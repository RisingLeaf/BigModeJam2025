...
class_name HookPointPlacerMain

@export var PlayerInst   : Player
@export var TileMapInst  : TileMapLayer
...
func is_field_free(pos : Vector2) -> bool:
	var grid_scale = Vector2(TileMapInst.tile_set.tile_size) * TileMapInst.scale
	var grid_pos = Vector2i(floor(pos / grid_scale))
	return TileMapInst.get_cell_source_id(grid_pos) == -1
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton and attached and is_field_free(Camera.get_global_mouse_position()):
